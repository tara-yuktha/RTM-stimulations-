...
# Reference moles per 100 g for normalization scalers (CRBG Grande Ronde Basalt baseline).
# For FIBG GOT13 (Bamberg et al. 2023): Ca=0.1892, Mg=0.1618, Fe=0.2481,
# Al=0.2658, Si=0.7924 mol/100g — scalers will be >1 for Ca/Mg/Fe (enriched).
...
# ---------------------------------------------------------------------------
# CARBON MASS BALANCE LEDGER  (R1 comment #10 — auditable closure error)
# ---------------------------------------------------------------------------
from dataclasses import dataclass, field

@dataclass
class CarbonBalanceRecord:
    """
    Per-step carbon mass balance ledger.

    All quantities in mol (field-scale, not mol/kgw).
    Use closure_error_pct to verify mass conservation — a value close to 0%
    confirms that injected CO₂ is fully accounted for across all sinks.

    Added to address Reviewer 1 comment #10: the 78%/52% headline efficiency
    figures need an auditable closure check alongside each reported value.
    """
    injected_mol:  float
    aqueous_mol:   float
    mineral_mol:   float
    exported_mol:  float
    gas_phase_mol: float = 0.0

    @property
    def closure_error_pct(self):
        """Percentage closure error (should be ~0% for a well-balanced simulation)."""
        total = (self.aqueous_mol + self.mineral_mol
                 + self.exported_mol + self.gas_phase_mol)
        if self.injected_mol <= 0:
            return float('nan')
        return 100.0 * (self.injected_mol - total) / self.injected_mol

    def summary(self):
        """Return a human-readable summary string."""
        return (
            f"Injected={self.injected_mol:.3e} mol | "
            f"Aqueous={self.aqueous_mol:.3e} ({self.aqueous_mol/max(self.injected_mol,1)*100:.1f}%) | "
            f"Mineral={self.mineral_mol:.3e} ({self.mineral_mol/max(self.injected_mol,1)*100:.1f}%) | "
            f"Exported={self.exported_mol:.3e} | "
            f"GasPhase={self.gas_phase_mol:.3e} | "
            f"Closure={self.closure_error_pct:.2f}%"
        )
...
try:
    from co2_solubility import (co2_solubility_mmol_kgw as _sol_ext,
                                nacl_molality_from_cl_mmol)
    _SOLUBILITY_MODULE = True
except ImportError:
    _SOLUBILITY_MODULE = False

SOLUBILITY_MODEL = "legacy_henry"   # "legacy_henry" | "spycher" | "henry"
PRESSURE_MODE    = "overpressure"   # "overpressure" | "bottomhole"
SOLUBILITY_CAP_MMOL = 180.0         # legacy numerical ceiling; None disables
BRINE_NACL_MOLAL = 0.0              # set by the engine from Cl_mmol


def co2_solubility_mmol_kgw(T_C, P_bar, model=None, m_NaCl=None):
    """
    CO2 solubility in the aqueous phase (mmol per kg of water).

    Dispatches to the model named by ``model`` (default: module-level
    SOLUBILITY_MODEL).
    """
    mdl = model or SOLUBILITY_MODEL
    salt = BRINE_NACL_MOLAL if m_NaCl is None else m_NaCl

    if mdl == "legacy_henry" or not _SOLUBILITY_MODULE:
        T_K   = T_C + 273.15
        T_ref = 313.15
        K_ref = 0.580          # KNOWN LOW BY ~40x - kept for reproducibility only
        Ea_R  = 2300.0
        K     = K_ref * math.exp(Ea_R * (1.0 / T_ref - 1.0 / T_K))
        return min(K * P_bar, 180.0)

    cap = SOLUBILITY_CAP_MMOL if mdl == "legacy_henry" else None
    return _sol_ext(T_C, P_bar, m_NaCl=salt, model=mdl, cap_mmol=cap)


def thermodynamic_pressure_bar(P_absolute_bar, depth_m, overpressure_bar,
                               mode=None):
    """
    Return the pressure that should be used in the CO2 solubility / pCO2
    calculation, according to PRESSURE_MODE.

    Parameters
    ----------
    P_absolute_bar   : the dynamic absolute pressure from pressure_at_time()
    depth_m          : injection depth (m)
    overpressure_bar : wellhead / injection overpressure above hydrostatic (bar)
    """
    m = mode or PRESSURE_MODE
    if m == "overpressure":
        return max(overpressure_bar, 1.0)
    return max(P_absolute_bar, 1.0)
...
def darcy_velocity_m_yr(permeability_mD, delta_P_bar, column_length_m,
                        mu_Pa_s=_MU_BRINE_PA_S, geometry="linear",
                        r_well_m=0.1, porosity_frac=None, trace=False):
    """
    Darcy (specific-discharge) velocity in m/yr.

    LINEAR (geometry="linear"):
        v_D = (k / mu) * (dP / L)

    RADIAL (geometry="radial"):
        v_D(r) = (k / mu) * dP / (r * ln(r_out / r_well))
        Local specific discharge at the geometric-mean radius:
        r* = sqrt(r_well * r_out)

    trace=True returns (v_myr, steps_dict) with full intermediate quantities.
    """
    k = permeability_mD * _MD_TO_M2                 # mD -> m^2
    dP = delta_P_bar * 1e5                          # bar -> Pa

    if geometry == "radial":
        r_out = max(column_length_m, r_well_m * 1.001)
        r_ref = math.sqrt(r_well_m * r_out)         # geometric-mean radius
        grad = dP / (r_ref * math.log(r_out / r_well_m))   # Pa/m
    else:
        grad = dP / column_length_m                 # Pa/m

    v_mps = (k / mu_Pa_s) * grad                    # m/s
    v_myr = v_mps * _SEC_PER_YR                     # m/yr
    ...
    steps = {
        "geometry":            geometry,
        "k_mD":                permeability_mD,
        "k_m2":                k,
        "delta_P_bar":         delta_P_bar,
        "delta_P_Pa":          dP,
        "length_m":            column_length_m,
        "pressure_gradient_Pa_per_m": grad,
        "mu_Pa_s":             mu_Pa_s,
        "mobility_m2_per_Pa_s": k / mu_Pa_s,
        "v_m_per_s":           v_mps,
        "seconds_per_year":    _SEC_PER_YR,
        "v_m_per_yr":          v_myr,
    }
    if porosity_frac:
        steps["porosity_frac"] = porosity_frac
        steps["pore_velocity_m_per_yr"] = v_myr / porosity_frac
    return v_myr, steps
...
def _build_eq_phases(saponite=True, current_eq_mol_kgw=None,
    ...
    if not carbonate_kinetic:
        _cal_seed = max(_mol('Calcite'), 1e-8)
        if not delay_calcite:
            s += f"    Calcite         0.0   {_cal_seed + carb_cap:.8e}\n"
    ...
    if not skip_clays:
        ...
        _clino_cap = _mol('Clinochlore-14A') + carb_cap * 0.25
        seed_ka = max(_mol('Kaolinite'), 1e-10)
        s += f"    Clinochlore-14A 0.0   {_clino_cap:.8e}\n"
        ...
        # FIBG GOT13 has K2O=0.22% → pw_K_mmol ~0.19 mmol/kgw — extremely K-poor.
        # Gate: suppress Muscovite if pw_K_mmol < 1.5 mmol/kgw (too K-poor).
        # This prevents spurious Muscovite formation in K-poor rocks (MHOW, FIBG)
        # and ensures it only forms where geochemically appropriate.
        # For FIBG: Muscovite is definitely suppressed (K2O only 0.22 wt%).
        # Expected FIBG secondary minerals: Calcite, Siderite/Ankerite (carbonate),
        # Saponite-Mg, Clinochlore-14A, Kaolinite (clays — Al2O3=13.54 is moderate).
...
def _build_transport_block(n_cells, dt_years, cell_length_m,
    ...
    advect_dist = flow_vel_m_yr * dt_years
    flow_dir   = "forward" if advect_dist >= 0.05 else "diffusion_only"
    ...
    block += f"    -flow_direction {flow_dir}\n"
...
# POROSITY
# ---------------------------------------------------------------------------
def porosity_from_saturation(w_saturated, w_dry, bulk_volume,
                             rho_water=1.0):
    """
    Calculate porosity (%) from a water-saturation gravimetric experiment.

    Corrects the dimensional error in the manuscript's Equation 1:
    the raw mass difference (W_sat - W_dry) has units of grams, and dividing
    by the fluid density [g/cm³] converts it to a pore volume [cm³].
    Dividing by the bulk volume [cm³] then gives the dimensionless porosity
    fraction, which is multiplied by 100 to express as a percentage.

    φ = [(W_sat − W_dry) / ρ_water] / V_bulk × 100

    Parameters
    ----------
    w_saturated : float — saturated sample mass (g)
    w_dry       : float — oven-dry sample mass (g)
    bulk_volume : float — bulk (geometric) sample volume (cm³)
    rho_water   : float — saturating fluid density (g/cm³); default 1.0

    Returns
    -------
    porosity : float — porosity in percent (%)

    Notes
    -----
    Added to address Reviewer 2 comment #4: the printed Equation 1 had
    (W_sat - W_dry) / V_bulk without the ρ_water denominator, yielding
    units of g/cm³ (a density) rather than the dimensionless porosity.
    This helper is not called at runtime — initial_porosity is a fixed
    CONFIG input — but it documents the correct formula and provides a
    standalone utility for cross-checking the reported core-plug values.
    """
    pore_volume = (w_saturated - w_dry) / rho_water
    return (pore_volume / bulk_volume) * 100.0
...
class PhreeqcEngine:
    ...
    def __init__(self, database_path, xrf_data, reg_params):
        ...
        # Solver-mode census (Reviewer 1 C11)
        self.solver_log = []
        self.batch_census = {
            'n_steps': 0, 'n_transport': 0,
            'n_batch_high_pco2': 0,
            'n_batch_large_dt': 0,
            'n_batch_fallback': 0,
            'n_batch_disabled': 0,
        }
        self._c_export_mol = 0.0
        self.carbon_log = []
    ...
    def step(self, pressure_bar, co2_mol_this_step, dt_years,
        ...
        # ── Independent Carbon Mass Balance Audit (Reviewer 1 Comment 10) ──
        _dic_mmol_kgw = p.get('total_DIC', float('nan'))
        _pk_c = self.pore_kg
        c_aqueous_mol = (_dic_mmol_kgw * 1e-3 * _pk_c
                         if _dic_mmol_kgw == _dic_mmol_kgw else float('nan'))

        # Advective export estimate: fraction of the column pore volume
        # displaced this step, times the outlet-cell DIC.
        _col_len = max(self.reg_params.get('transport_col_len_m', 50.0), 1e-9)
        _v = self._vel_m_yr if self._vel_m_yr else 0.0
        _phi_frac = max(self._porosity_current, 1e-6) / 100.0
        _pore_vel = _v / _phi_frac                       # interstitial velocity
        _displaced_frac = min(max(_pore_vel * dt_years / _col_len, 0.0), 1.0)
        _sp_dic = p.get('spatial_DIC', [])
        _dic_out_mmol = (_sp_dic[-1] if _sp_dic and _sp_dic[-1] == _sp_dic[-1]
                         else (_dic_mmol_kgw if _dic_mmol_kgw == _dic_mmol_kgw else 0.0))
        # Export can only occur on steps that actually ran the TRANSPORT module;
        # batch (single-cell) steps have no outlet flux by construction.
        _ran_transport = (getattr(self, '_solver_reason_this_step', '')
                          == 'transport')
        _export_step = (_displaced_frac * _pk_c * _dic_out_mmol * 1e-3
                        if _ran_transport else 0.0)
        self._c_export_mol += max(_export_step, 0.0)

        c_injected = self._total_co2
        c_initial  = getattr(self, '_initial_dic_mol', 0.0)
        c_free     = getattr(self, '_co2_excess_mol', 0.0)
        c_mineral  = co2_seq
        _sum_sinks = ((c_aqueous_mol if c_aqueous_mol == c_aqueous_mol else 0.0)
                      + c_mineral + c_free + self._c_export_mol)
        c_residual = (c_injected + c_initial) - _sum_sinks
        closure_err_pct = (abs(c_residual) / max(c_injected + c_initial, 1.0) * 100.0)

        self.carbon_log.append({
            'step':        self._step,
            'dt_yr':       dt_years,
            'C_injected':  c_injected,
            'C_initial':   c_initial,
            'C_aqueous':   c_aqueous_mol,
            'C_mineral':   c_mineral,
            'C_free':      c_free,
            'C_export':    self._c_export_mol,
            'C_residual':  c_residual,
            'closure_err_pct': closure_err_pct,
            'solver_mode': getattr(self, '_solver_reason_this_step', 'n/a'),
        })
        ...
        res = {
            ...
            # Reviewer 1 C10: carbon mass balance quantities
            'C_injected_mol':  c_injected,
            'C_initial_mol':   c_initial,
            'C_aqueous_mol':   c_aqueous_mol,
            'C_mineral_mol':   c_mineral,
            'C_free_mol':      c_free,
            'C_export_mol':    self._c_export_mol,
            'C_residual_mol':  c_residual,
            'closure_err_pct': closure_err_pct,
            'total_DIC_mmol':  _dic_mmol_kgw,
        }
        for m in CARBONATE_MINERALS + CLAY_MINERALS:
    ...
    def _run_reactive(self, dt_years, pressure_bar=8,
        ...
        """
        is_injection   = (co2_mol_per_kgw > 0)
        ...
        # Record why this step used batch or transport (Reviewer 1 C11)
        self.batch_census['n_steps'] += 1
        if not self._use_transport:
            _mode_reason = 'batch_disabled'
            self.batch_census['n_batch_disabled'] += 1
        elif _high_pco2:
            _mode_reason = 'batch_high_pco2'
            self.batch_census['n_batch_high_pco2'] += 1
        elif _large_dt:
            _mode_reason = 'batch_large_dt'
            self.batch_census['n_batch_large_dt'] += 1
        else:
            _mode_reason = 'transport'
            self.batch_census['n_transport'] += 1
        self._solver_reason_this_step = _mode_reason
        ...
        if n_err > 0 and use_transport_this_step:
            ...
            # Reclassify from transport to batch_fallback
            self.batch_census['n_transport'] = max(self.batch_census['n_transport'] - 1, 0)
            self.batch_census['n_batch_fallback'] += 1
            self._solver_reason_this_step = 'batch_fallback'
            if self.solver_log:
                self.solver_log[-1]['mode'] = 'batch_fallback'
    ...
    def _build_transport_input(self, dt_years, log_pco2_eff, delay_calcite,
        ...
        if is_injection and co2_mol_per_kgw > 0:
    ...
    def _parse_output(self):
        ...
        try:
            ...
            # Data rows: select final state rows for column cells (FIX v30:
            # prevent averaging intermediate kinetics steps and unreacted shifts)
            all_rows  = arr[1:]
            state_col = hmap.get('state')
            step_col  = hmap.get('step')
            transp_rows = [r for r in all_rows if state_col is not None and str(r[state_col]).strip().lower() == 'transp']
            if transp_rows:
                max_step = max(int(float(r[step_col])) for r in transp_rows)
                data_rows = [r for r in transp_rows if int(float(r[step_col])) == max_step]
            else:
                react_rows = [r for r in all_rows if state_col is not None and str(r[state_col]).strip().lower() == 'react']
                if react_rows:
                    data_rows = [react_rows[-1]]
                else:
...
def run_sensitivity(run_fn, base_config, region, mode,
    ...
    param_names  : list of str — parameters to perturb.
                   Default includes the critical parameters flagged by
                   Reviewer 1 (#5) and Reviewer 2 (#14):
                     'access_fraction'           — reactive-access fraction (0.25)
                     'max_kinetic_mol_per_step'  — per-step dissolution cap (1e-3)
                     'carb_cap_base'             — per-step carbonate cap base (9e-5)
                   plus the original set: T_C, initial_porosity,
                   dissolution_boost, permeability_mD.
    factors      : list of float — perturbation factors.
                   Default: [0.5, 0.8, 1.0, 1.25, 2.0]
                   (includes 0.5× and 2.0× to bracket an order-of-magnitude
                    sweep, as requested for the dissolution cap in R1 #6)
    ...
    Notes
    -----
    The three new parameters (access_fraction, max_kinetic_mol_per_step,
    carb_cap_base) are read from CONFIG["kinetics"] rather than
    region_params. The sweep modifies both locations for consistency.
    """
    import copy

    # --- Parameters that live in CONFIG["kinetics"] rather than region_params ---
    _KINETICS_PARAMS    = {'access_fraction', 'max_kinetic_mol_per_step', 'carb_cap_base'}
    # --- Parameters that live in CONFIG["geochemistry"] ---
    # log_pco2_off_floor: swept with absolute additive offsets, not multiplicative factors.
    # factor=0.5 → floor+0.5 (shallower flush); factor=1.0 → no change; factor=2.0 → floor-0.5
    _GEOCHEMISTRY_PARAMS = {'log_pco2_off_floor'}

    if param_names is None:
        param_names = [
            # Rock/reservoir properties
            'T_C', 'initial_porosity', 'dissolution_boost', 'permeability_mD',
            # Critical kinetic numerical parameters (R1 #5 / R2 #14)
            'access_fraction',           # reactive surface fraction (0.25)
            'max_kinetic_mol_per_step',  # per-step dissolution cap (1e-3 mol/kgw/step)
            'carb_cap_base',             # per-step carbonate cap base (9e-5 mol/kgw)
            # Geochemical boundary condition (R1 #2 / R1 #7)
            'log_pco2_off_floor',        # residual pCO2 floor after water flush (-2.5)
        ]
    if factors is None:
        # Wider range (0.5×–2.0×) to bracket the order-of-magnitude test
        # For log_pco2_off_floor: factor is an additive offset multiplier;
        #   factor=0.5 → base+0.50 (shallower, -2.0); factor=1.0 → base (-2.5);
        #   factor=1.5 → base-0.25 (-2.75); factor=2.0 → base-0.50 (-3.0)
        factors = [0.50, 0.80, 1.00, 1.25, 2.00]
    ...
    for pname in param_names:
        for fac in factors:
            ...
            kin = cfg.get('kinetics', {})
            geo = cfg.get('geochemistry', {})
            _perturbed_val = float('nan')

            if pname in _GEOCHEMISTRY_PARAMS:
                # For log_pco2_off_floor: additive offset = (1 - factor) * 0.5
                # so factor=0.5 → base+0.50 (shallower, -2.0); factor=1.0 → base (-2.5);
                #    factor=1.25→ base-0.125 (-2.625); factor=2.0 → base-0.50 (-3.0)
                if pname not in geo:
                    print(f"  [SENS] Skipping '{pname}' — not in CONFIG['geochemistry']")
                    continue
                old_val = geo[pname]
                geo[pname] = old_val + (1.0 - fac) * 0.5
                cfg['geochemistry'] = geo
                _perturbed_val = geo[pname]
                print(f"  [SENS] geochemistry['{pname}'] offset {fac:.2f}: "
                      f"{old_val:.2f} → {geo[pname]:.2f}")
            elif pname in _KINETICS_PARAMS:
                # Parameter lives in CONFIG["kinetics"]
                if pname not in kin:
                    print(f"  [SENS] Skipping '{pname}' — not in CONFIG['kinetics']")
                    continue
                old_val = kin[pname]
                kin[pname] = old_val * fac
                cfg['kinetics'] = kin
                _perturbed_val = kin[pname]
                print(f"  [SENS] kinetics['{pname}'] × {fac:.2f}: {old_val:.3g} → {kin[pname]:.3g}")
            elif pname in rp:
                old_val = rp[pname]
                rp[pname] = old_val * fac
                _perturbed_val = rp[pname]
                print(f"  [SENS] {pname} × {fac:.2f} = {rp[pname]:.4g}")
            else:
                print(f"  [SENS] Skipping '{pname}' — not in {region} params, kinetics, or geochemistry")
                continue
            try:
                ...
                results.append({
                    'param':            pname,
                    'factor':           fac,
                    'perturbed_value':  _perturbed_val,
                    'efficiency_pct':   float(r.get('efficiency_arr', [float('nan')])[-1]),
                    'pH_nadir':         float(r['pH'].min()),
                    'pH_final':         float(r['pH'][-1]),
                    'CO2_seq_mol_kgw':  float(r['co2_seq_kgw'][-1]),
                    'porosity_final':   float(r['porosity'][-1]),
                    'Calcite_final':    float(r['minerals'].get('Calcite', [float('nan')])[-1]),
                    'Ankerite_final':   float(r['minerals'].get('Ankerite', [float('nan')])[-1]),
                })
            except Exception as e:
...
# ===========================================================================
# ── WALLULA BENCHMARKING CHECK  [White et al. 2020]
# (Renamed from calibrate_to_wallula → benchmark_to_wallula, R1 #12 / R2 #7)
# ===========================================================================
def benchmark_to_wallula(results_p):
    """
    Compare simulation output against key Wallula pilot field observations.

    Renamed from 'calibrate_to_wallula' to 'benchmark_to_wallula' (R1 #12).
    The comparison is a qualitative benchmarking exercise, not independent
    quantitative validation — consistent with the terminology used in the
    ...
    This function prints a pass/fail benchmarking report and returns metrics.
    ...
    # efficiency_2yr: fraction of injected CO₂ mineralised by 2 yr post-injection end.
    # Use the pre-computed efficiency_arr (already = mineral_mol/injected_mol × 100)
    # sampled at the ~2yr post-injection mark, which avoids unit-mismatch errors.
    inj_end_t   = r.get('inj_window', r.get('T_inj_yr', 0.082))
    t_target    = inj_end_t + 2.0
    eff_arr     = r.get('efficiency_arr', None)
    if eff_arr is not None and len(eff_arr) > 0:
        mask_2yr = t <= t_target
        eff_2yr  = float(eff_arr[mask_2yr][-1]) if np.any(mask_2yr) else float(eff_arr[-1])
    else:
        # fallback: compute from seq and total injected mol
        inj_mol_total = r.get('co2_injected_total', inj)
        mask_2yr = t <= t_target
        seq_at   = seq[mask_2yr][-1] if np.any(mask_2yr) else seq[-1]
        pore_kg  = r['reg_params'].get('pore_kg_approx', None)
        if pore_kg and pore_kg > 0 and inj_mol_total > 0:
            eff_2yr = (seq_at * pore_kg / inj_mol_total) * 100.0
        else:
            eff_2yr = 0.0
    ...
    print("\n" + "=" * 68)
    print("WALLULA BENCHMARKING CHECK  [White et al. 2020 / Schaef & McGrail 2009]")
    print("(Benchmarking against literature range — not independent validation)")
    print("=" * 68)
    all_pass = True
    ...
    print("  Engine: PHREEQC v20 | EQ-phase carbonates | Duan & Sun CO₂ solubility")
    print("=" * 68)
...
# ---------------------------------------------------------------------------
# Backward-compatibility alias (preserves existing call-sites)
# ---------------------------------------------------------------------------
calibrate_to_wallula = benchmark_to_wallula
